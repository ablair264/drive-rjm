...
const AdminLogin = ({ onLogin }) => {
  ...
  return (
    <div className="min-h-screen bg-light-grey flex items-center justify-center px-4">
      <div className="bg-white rounded-2xl shadow-2xl border border-gray-200 w-full max-w-md p-8 space-y-6">
        <div className="text-center">
          <div className="inline-flex items-center gap-2 bg-learner-red text-white px-4 py-2 rounded-full text-sm font-bold tracking-wide">
            ADMIN ACCESS
          </div>
          <h1 className="text-3xl font-display font-bold text-dark mt-4">Drive RJM Admin</h1>
          <p className="text-medium-grey mt-1 text-sm">Enter your credentials to continue.</p>
        </div>
...
const AdminPage = ({ passes, enquiries, onAddPass, onLogout }) => {
  const navItems = [
    { label: 'Dashboard', Icon: LayoutDashboard },
    { label: 'Enquiries', Icon: Inbox },
    { label: 'Lesson Planner', Icon: CalendarClock },
  ...
  const computeRecommendedSlot = () => {
    const occupied = lessons
      .filter((lesson) => lesson.date === lessonForm.date)
      .map((lesson) => lesson.slot)

    return standardLessonSlots.find((slot) => {
      const [start, end] = slot.split('-').map((t) => {
        const [h, m] = t.split(':').map(Number)
        return h * 60 + m
      })
      return !occupied.some((busy) => {
        const [bs, be] = busy.split('-').map((t) => {
          const [h, m] = t.split(':').map(Number)
          return h * 60 + m
        })
  ...
  const fuelEstimate = () => {
    const mpg = 47
    const miles = Number(lessonForm.miles) || 0
    const gallons = miles / mpg
    const litresUsed = gallons * 4.546
    const tankLitres = 45
    const percent = Math.max(0, Math.min(100, ((tankLitres - litresUsed) / tankLitres) * 100))
    return {
      litresUsed: litresUsed.toFixed(1),
      tankPercent: percent.toFixed(0)
    }
  }

  const addLesson = () => {
    const slot = lessonForm.slot || computeRecommendedSlot()
    const newLesson = {
      ...lessonForm,
      slot
    }
  ...
  const openLessonModal = () => {
    setLessonForm((prev) => ({ ...prev, slot: computeRecommendedSlot() }))
  ...
  const handleEnquiryModalSubmit = (e) => {
    e.preventDefault()
    handleAddEnquiry({
      ...enquiryModalData,
      source: 'admin-modal',
  ...
  const lessonsForDate = (date) => {
    const key = date.toISOString().split('T')[0]
    return lessons.filter((lesson) => lesson.date === key)
  }
  ...
  const renderContent = () => {
      ...
      return (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
            <div>
              <div className="text-sm font-semibold text-medium-grey uppercase tracking-wide">Enquiries</div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200 text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {enquiries.length === 0 && (
                  <tr>
                    <td colSpan="6" className="px-4 py-4 text-medium-grey text-center">No enquiries yet.</td>
                  </tr>
                )}
                {enquiries.map((q, idx) => (
    ...
    if (active === 'Lesson Planner') {
      const days = getCalendarDays()
      const viewDate = plannerDate.toISOString().split('T')[0]
      const filteredLessons = lessons.filter((lesson) => {
        const matchesDate = lesson.date === viewDate
        const query = lessonSearch.toLowerCase()
        const matchesSearch = lesson.student.toLowerCase().includes(query)
      ...
      return (
        <div className="space-y-6">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200">
            <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
              <div>
                <div className="text-sm font-semibold text-medium-grey uppercase tracking-wide">Lesson Planner</div>
                <h3 className="text-xl font-display font-bold text-dark">Calendar & search</h3>
              </div>
              <span className="text-sm text-medium-grey">Base: WR6 6HU</span>
            </div>
            <div className="p-6 space-y-6">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                <div className="text-sm font-semibold text-dark">Recommended slot: {computeRecommendedSlot()}</div>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => setPlannerDate(new Date(plannerDate.getFullYear(), plannerDate.getMonth() - 1, 1))}
                    className="px-3 py-2 border border-gray-300 rounded-lg hover:border-learner-red transition-colors"
                  >
                    Prev
                  </button>
                  <button
              ...
              <div className="grid lg:grid-cols-2 gap-6">
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <h4 className="text-lg font-bold text-dark">
                      {plannerDate.toLocaleString('default', { month: 'long' })} {plannerDate.getFullYear()}
                    </h4>
                    <span className="text-xs text-medium-grey">Lesson counts</span>
                  </div>
                  ...
                  <div className="grid grid-cols-7 gap-2 mt-2 text-xs">
                    {days.map(({ day, date }, index) => {
                      const count = date ? lessonsForDate(date).length : 0
                      return (
                        <div
                          key={`${day}-${index}`}
                          className={`min-h-[70px] rounded-xl border ${day ? 'bg-white' : 'bg-gray-50'} border-gray-200 p-1 relative`}
                        >
                          <div className="text-right font-bold text-gray-700">{day || ''}</div>
                          {count > 0 && (
                            <span className="absolute bottom-1 left-1/2 -translate-x-1/2 rounded-full bg-learner-red px-2 text-white text-[10px]">
                ...
                <div className="space-y-4">
                  <div>
                    <label className="text-sm font-semibold text-dark">Filter by student</label>
                    <input
                      className="mt-1 w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-learner-red"
                  ...
                  <div className="max-h-96 overflow-y-auto space-y-3">
                    {filteredLessons.length === 0 && (
                      <div className="text-sm text-medium-grey">No lessons for that search.</div>
                    )}
                    {filteredLessons.map((lesson) => (
                      <div key={lesson.id} className="p-3 bg-white rounded-xl border border-gray-200 shadow-sm space-y-1">
                        <div className="flex justify-between text-sm font-semibold text-dark">
                          <span>{lesson.student || 'Student'}</span>
                          <span className="text-learner-red">{lesson.slot}</span>
                        </div>
                        <div className="text-xs text-medium-grey">
                          {lesson.startPostcode || 'WR6 6HU'} → {lesson.endPostcode || 'WR6 6HU'}
                        </div>
                        <div className="text-xs text-medium-grey">
                          {lesson.duration} mins • ~{lesson.miles} miles
                        </div>
                      </div>
                    ))}
                  </div>
    ...
    return (
      <>
        <div className="grid md:grid-cols-3 gap-4">
          {[
            { title: 'Open Enquiries', value: enquiries.length.toString(), sub: 'Live submissions', Icon: Inbox },
            { title: 'Lessons This Week', value: '24', sub: '6 new learners', Icon: CalendarClock },
            { title: 'Upcoming Tests', value: '5', sub: 'Next: Fri 10:30', Icon: ClipboardCheck }
          ].map((card) => {
            const IconComponent = card.Icon
            return (
              <div key={card.title} className="bg-white border-l-4 border-learner-red rounded-xl shadow-sm p-5 flex items-center gap-4">
                <div className="h-12 w-12 bg-learner-red text-white rounded-full flex items-center justify-center shadow-md">
                  <IconComponent className="w-6 h-6" />
                </div>
                <div>
                  <div className="text-sm text-medium-grey font-semibold uppercase tracking-wide">{card.title}</div>
        ...
        <div className="grid lg:grid-cols-5 gap-6">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="/driving-instructor-worcester.webp or paste Supabase URL"
                />
                <div className="mt-2 flex items-center gap-3">
                  ...
                  <span className="text-xs text-medium-grey">Will store locally for now; swap to Supabase upload when ready.</span>
                </div>
                {formState.image && (
                  <div className="mt-3">
          ...
          <div className="lg:col-span-3 bg-white rounded-xl shadow-sm border border-gray-200">
            ...
            <div className="p-6 grid md:grid-cols-2 gap-4">
              {passes.map((p, i) => (
                <div key={`${p.name}-${i}`} className="border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                  <div className="h-32 bg-gray-100">
                    <img src={p.image || '/driving-instructor-worcester.webp'} alt={p.name} className="w-full h-full object-cover" />
                  </div>
                  <div className="p-4 space-y-1">
                    <div className="text-lg font-bold text-dark">{p.name}</div>
                    {p.tests && <div className="text-sm font-semibold text-learner-red uppercase tracking-wide">{p.tests}</div>}
                    ...
                    <div className="flex gap-2 mt-3">
                      ...
                      <button
                        ...
                        className="px-3 py-2 text-xs font-semibold border border-red-500 text-red-600 hover:bg-red-600 hover:text-white transition-colors"
                      >
                        Delete
                      </button>
                    </div>
              ...
              {!passes.length && (
                <div className="text-medium-grey text-sm">No passes added yet.</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
            <div>
              <div className="text-sm font-semibold text-medium-grey uppercase tracking-wide">Today</div>
              <h3 className="text-xl font-display font-bold text-dark">Schedule & Activity</h3>
            </div>
            <button className="text-sm font-semibold text-learner-red hover:text-dark">View calendar</button>
          </div>
          <div className="p-6 grid md:grid-cols-2 gap-4">
            <div className="bg-light-grey rounded-lg p-4 border border-gray-200">
              <div className="flex items-center gap-3 mb-3">
                <CalendarClock className="w-5 h-5 text-learner-red" />
                <div>
                  <div className="text-sm text-medium-grey uppercase font-semibold tracking-wide">Lessons</div>
                  <div className="text-lg font-bold text-dark">6 scheduled today</div>
                </div>
              </div>
              <p className="text-medium-grey text-sm">Next: 10:00 — City routes & reverse bay practice.</p>
            </div>
            <div className="bg-light-grey rounded-lg p-4 border border-gray-200">
              <div className="flex items-center gap-3 mb-3">
                <ClipboardCheck className="w-5 h-5 text-learner-red" />
                <div>
                  <div className="text-sm text-medium-grey uppercase font-semibold tracking-wide">Tests</div>
                  <div className="text-lg font-bold text-dark">2 tests this week</div>
                </div>
              </div>
              <p className="text-medium-grey text-sm">Prepare routes and mock tests for candidates.</p>
            </div>
        </div>
      </div>
    </>
